...
class CoCSPInterface(CLIPInterface):
    def __init__(
        ...
        self.soft_embeddings = soft_embeddings.to(self.device)
    ...
    def construct_token_tensors(self, batch_img, pair_idx):
        ...
        soft_embeddings[:, :self.offset, :] = soft_embeddings[:, :self.offset, :] + vctx[:, 0].unsqueeze(-1).unsqueeze(-1).repeat(1, self.offset, 1)
        soft_embeddings[:, self.offset:, :] = soft_embeddings[:, self.offset:, :] + vctx[:, 1].unsqueeze(-1).unsqueeze(-1).repeat(1, soft_embeddings.shape[1] - self.offset, 1)
